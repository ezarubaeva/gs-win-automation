...
public class TestLogger {
    ...
    private static final String[] emailListDefault = {"agolov@siber.com","p-ray@yandex.ru"};
    ...
    */
    //^ rf-cmd paths that are used to gain test build version
...
/*        try {
        ...
        //here shell command it executed to gain the version of the crrent test build, returning dummy value:
        return "1.1";
    }
...
/*        if (RunAllTests.isOfficial()){
        ...
        //^ could be RF-specific - if we can get build version, write it down, otherwise - write only time, so I place only time below:
        logh.add("  <title>Test summary for tests started from " + startDate.toString() + "</title>");
...
/*        if (RunAllTests.isOfficial()){
        ...
        //another thing related to gaining the build version, I place just official part here:
        filename = "REPORT_" + fd.format(startDate) + ".html";
