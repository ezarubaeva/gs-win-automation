...
public class SeleniumProc {
     ...
     * @param _elemDName target element description to look for (Name of element)
     * @param _elemDCName target element description to look for (ClassName of element)
     * @param _elemDAutId target element description to look for (AutomationID of element)
     ...
     */
    public static WebElement gimMeP(SearchContext _webElement, String _elemDName, String _elemDCName, String _elemDAutId, String _name, int _attempts, String _paramT, String _paramF) throws Exception{
        Proc.lgP = "gimMeP";
        log("Locating " + _name);
        WebElement res;
        ...
        if (_paramF.toUpperCase().contains("H")){
            if (hasKBFocusF == 1){
                cl();
        ...
        if (useXPath){
            ...
            String xpathTotal = xpathStart +
                    parseXPath(_elemDName, _elemDCName, _elemDAutId, removeName,removeCName,removeAID,isOffscreenF,isEnabledF,hasKBFocusF) + xpathEnd;
        ...
        {
            if (removeCName){
                log("Using find by name: "+_elemDName);
                hh = By.name(_elemDName);
            }
            if (removeName) {
                log("Using find by classname: "+_elemDCName);
                hh = By.className(_elemDCName);
            }
            if (hh == null){
                cl();
    ...
    private static String parseXPath(String _elemDName, String _elemDCName, String _elemDAutId, Boolean _removeName, Boolean _removeCName, Boolean _removeAID,
        ...
        if (!_removeCName){
            ...
            xpathArgs += "(@ClassName='" + _elemDCName + "')";
            wasItems = true;
        }
        if (!_removeAID){
            if (wasItems){
            ...
            xpathArgs += "(@AutomationId='" + _elemDAutId + "')";
            wasItems = true;
        }
        if (_isOffscreenF > 0){
            if (wasItems){
